/*! mccBackStretchCarousel - an AngularJS directive
 * Uses jQuery backstretch with an image selector, swipe effects and
 ...
 * Example use
 *  - in template
 *    <div ng-controller='temp' mcc-back-stretch-carousel></div>
 *
 *  - in controller
...
angular.module('mcc').directive('mccBackStretchCarousel', ['$swipe', function ($swipe) {
    ...
    directive.compile = function (element, attributes) {
      var linkFunction = function ($scope, element, attributes) {
        ...
        $scope.$watch('images', function (images) {          
          ...
          // add backstretch
          try {
            var viewFrame = $('.app-content')[0];
            var height = $(viewFrame).height();
            var width = $(viewFrame).width();
            $(element).css('height', height).css('width', width);
          } catch (ex) {
